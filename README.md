...
> 实验组与对照组在图视角下的本质区别：yₙ = fₙ(yₙ₋₁) + tanh(yₙ₋₁)的残差连接方式，在递归展开时使残差边嵌套多层tanh()，承担指数级梯度下降风险
...
- 50轮Loss&max_Acc
   ...
   - 在残差块堆叠50层时，实验组的残差边堆叠过多激活函数，残差边信息传递功能失效，主干虽无梯度衰减风险，却只能学习线性变换。
...
2. **坏死判断失效**
   - 当梯度范数<0.01判断为坏死，所有残差块坏死神经元率≈0%
   - **理论验证**：尽管梯度范数>0.01，但并不代表该神经元被正常训练
...
对照组 = 实验组2 >= 实验组1（0.4%）
```
**理论符合**：在低梯度消失风险时，原残差连接的信息传递优势微小提升训练速度，但性能无本质差距。

### 5.4 边的不可区分性(非实验结果)

- 当尝试设计实验组3，让残差路径承担所有线性变换职能时，发现无法与实验组1进行有效区分
- **哪一个才是残差项**：无法有效区分线性变换项与激活函数项，哪一个才是残差边，哪一个才是主干道。

> 在将信息处理与信息传递职能分散后，已无法定义图的残差边与主干道(后续解释中暂且定义，激活项为残差边)。

### 5.5 实验总结
信息传递与处理具有不兼容性，承担更多的信息处理职能会造成**梯度信息传播闭塞**，而不承担信息处理职能又会导致**梯度消失风险集中分布**，信息处理与传递的职能分布将影响神经网络的最终性能。
在20层时实验组凭借分担主干信息处理压力与梯度下降风险，提升整体性能；在50层由于残差边堆叠的激活函数，导致**残差边失效**，主干虽提供巨量梯度信息，却只能学习线性变换，导致损失无法下降。
