...
| $W$ 值域 | 梯度统计特性               | 训练表现               |
|----------|--------------------------|----------------------|
| $W \approx 1$ | 梯度均值和方差保持稳定      | 训练稳定收敛          |
| $W > 1$     | 梯度均值/方差随深度指数增大 | 更新步幅过大，难以收敛 |
| $W < 1$     | 梯度均值/方差随深度指数衰减 | 梯度消失，训练停滞    


#### 假设2：通畅度对非线性特征学习能力的影响

- 若两个模型的激活函数的种类与个数都相同，则通畅度小的模型拥有更强的非线性特征学习能力

具体表现：

- 浅层网络中，通畅度小的模型往往拥有更好的表现能力
- 深层网络中，随着梯度衰减风险增加，通畅度小的模型失去优势
...
graph LR
    subgraph 残差块结构
        ...
        subgraph 实验组2[Exp2：二次连接残差块]
            direction TB
            E21["output1=linear1(x)"] --> E22["output2=relu(output1+x)"] --> E23["output3=linear2(output2)"] --> E24["output4=relu(output3+output2)"] --> E25["y"]
        end

        subgraph 实验组1[Exp1：tanh残差激活残差块]
            direction TB
            E11["output1=linear1(x)"] --> E12["output2=tanh(output1)"] --> E13["output3=linear2(output2)"] --> E14["output4=tanh(output3+output2)"] --> E15["y"]
        end

        subgraph 对照组[Control：标准残差块]
            direction TB
            C11["output1=linear1(x)"] --> C12["output2=relu(output1)"] --> C13["output3=linear2(output2)"] --> C14["output4=relu(output3+output2)"] --> C15["y"]
        end
        
    end
```

由于归一化，dropout，初始化等原因，无法准确计算W的值。以下仅进行估算。

| 实验组 | 层通畅度估值              | 梯度情况             |
|----------|------------------|----------------------|
| 对照组    | 0.98           | 适应多种超参数，深层网络梯度略微衰减 |
| 实验组1     | <0.9         | 深层网络明显梯度衰减 |
| 实验组2     | >1.1         | 深层网络明显梯度增加    |
...
测量堆叠不同残差块数量时，三个实验组的梯度均值与方差。

### 5.3 验证假设2


测量堆叠不同残差块数量时，三个实验组的准确率峰值与损失最低值。
...
- **建立设计准则**：提出`W≈1`的梯度稳定条件，为网络架构设计提供严格数学指导  
...
> 要改变的，不是残差连接也不是神经网络，而是人们的思考方式——“世界要在科学的框架下运行”。
