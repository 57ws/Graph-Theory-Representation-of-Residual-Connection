...
- 构建了一个**边数为 $$\frac{n^2}{2}$$ 的梯度计算图**，有向无环
...
```mermaid
graph LR
    A[信息传递] -- 加法操作 --> B[不分担信息处理]
    C[信息处理] -- 复杂变换 --> D[梯度消失风险提升]
...
> 实验组与对照组的本质区别：yₙ = fₙ(yₙ₋₁) + tanh(yₙ₋₁)的残差连接结构，在递归展开时使残差边嵌套多层tanh()，分担指数级梯度下降风险
...
对照组 > 实验组2（0.1%） > 实验组1（2%）
```
**理论符合**：在低风险环境，原始设计的信息传递效率优势重新主导，实验组2平衡信息处理与传递，在多环境下性能良好。

### 5.4 实验总结
信息传递与处理具有不兼容性，承担更多的信息处理职能会造成**梯度信息传播闭塞**，而不承担信息处理职能又会导致**梯度消失风险集中分布**，信息处理与传递的职能分布将影响神经网络的最终性能。

> 证明了在不增加额外消耗的情况下，在特定环境中，仅修改图结构能使残差连接性能达到原始残差的104%。
