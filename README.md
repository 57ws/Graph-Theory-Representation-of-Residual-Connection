...
### 2.3 标准残差块递归推导
事实上，`yₙ = fₙ(yₙ₋₁) + yₙ₋₁`并不是标准的残差连接，标准残差连接应为`yₙ = gₙ(fₙ(yₙ₋₁) + yₙ₋₁)`,如`yₙ = ReLu(fₙ(yₙ₋₁) + yₙ₋₁)`。
...
### 3.1 推导多叉树的形成
如`yₙ = ReLu(fₙ(yₙ₋₁) + yₙ₋₁)`形态的残差块，梯度经过次残差块时，将会分为2条路径，一条为主干路径即`fₙ(yₙ₋₁)`，另一条为残差路径即`yₙ₋₁`。

每经过一层次类残差块，路径数*2。意味着，当该残差块堆叠了n层时，梯度的传递路径有x=2^n条。这些所有路径的梯度和则为最终传递到输出层的路径。

为了直观体现其影响，我们假设残差块有n=20层，此时梯度路径则一共有x = 1048576条，这使得梯度值远超链式路径的模型。

### 3.2 激活函数对梯度的衰减作用

我们不妨假设，梯度经过一个激活函数时，梯度会变为原来的k(<1)倍，当经过了n个激活函数时，梯度变为原来的k^n倍，随着经过的激活函数越多，梯度损失越多。
...
**不同激活函数组合的衰减对比**（`L=20`）：
| 激活函数组合 | σ' | σ₁' | 全主干路径衰减 | 全残差路径衰减 | 衰减差异倍数 |
|--------------|----|-----|----------------|----------------|--------------|
| ReLU+ReLU    | 1  | 0   | 1²⁰×0²⁰=0      | 1²⁰=1          | -            |
| tanh+tanh    | 0.8| 0.8 | 0.8²⁰×0.8²⁰≈1.2×10⁻⁴ | 0.8²⁰≈0.0115  | 9.2×10⁻³     |
| ReLU+Swish   | 1  | 0.3 | 1²⁰×0.3²⁰≈3.5×10⁻¹¹ | 1²⁰=1          | 3.5×10⁻¹¹    |
...
- **实验设计**：
  ...
  | 组合          | 通畅度因子 | 预期准确率 | 预期收敛速度 | 特征非线性 |
  |---------------|------------|------------|--------------|------------|
  | ReLU+ReLU     | 1.0        | 最高       | 中等         | 最强       |
  | ReLU+Swish    | 1.3        | 中等       | 最快         | 中等       |
  | tanh+ReLU     | 0.8        | 最低       | 最慢         | 较强       |
